...
Condensed reference from Edward Tufte's *The Visual Display of Quantitative Information*. Use this when designing or critiquing dashboards, analytics screens, in-product charts, and exported reports. The copilot and `/tufte-viz` skill pull from here for chart integrity, data-ink, and density questions.
...
- [`semantic-color.md`](semantic-color.md) — Color encoding vs decoration in charts; semantic meaning in UI
- [`wcag.md`](wcag.md) — Alt text for charts, contrast, color independence (WCAG 1.4.1)
- [`refactoring-ui-principles.md`](refactoring-ui-principles.md) — Visual hierarchy and label management for mixed dashboard layouts
